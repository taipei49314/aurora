...
| 37 | Docker compose one-command up (§4) | **PARTIAL** | `docker-compose.yml`, Dockerfiles, `scripts/docker_audit.py` | static contract audit passes; image build / startup NOT verified (no Docker on the build machine) |
...
- **Docker** compose files exist and their checked-in contract is covered by
  `scripts/docker_audit.py`, but the stack was not run (no Docker on the build
  box — still the only PARTIAL left, environment-blocked).
